...
must_haves:
  truths:
    ...
    - "Database has files, path_index, and settings tables with correct schema (unit test verifies via sqlite_master)"
    - "Settings can be read and written via Rust store functions (unit test verifies round-trip)"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **capabilities/default.json**: Minimal permissions — `core:default`, `core:window:default`, and `dialog:default` (for native folder pickers).

    **main.rs**: Tauri builder with setup hook that calls `store::connection::init_db()` using `app.path().app_data_dir()`, creates AppState, registers it with `app.manage()`. Register `tauri_plugin_dialog::init()` plugin. Register empty command handler list (commands added in Plan 03). Include `mod commands; mod services; mod parser; mod store; mod state;`.
    ...
    **package.json**: Name "flp-vault", version "0.1.0", with `@tauri-apps/api` (version ^2) and `@tauri-apps/plugin-dialog` (version ^2) dependencies.
...
<task type="auto">
  ...
  <action>
    ...
    **Unit tests in connection.rs**: Add a `#[cfg(test)] mod tests` block with at least:
    - `test_init_db_creates_tables`: Call `init_db()` with a `tempfile::tempdir()` path, then query `sqlite_master` to assert all three tables (`files`, `path_index`, `settings`) exist.
    - `test_settings_defaults`: Call `init_db()` then `get_all_settings()` and verify smart defaults are returned (source_folder, organized_folder, originals_folder are non-empty).
    - `test_set_and_get_setting`: Call `set_setting()` then `get_setting()` and verify round-trip.
  </action>
  <verify>
    <automated>cd src-tauri && cargo test store 2>&1 | tail -10 && cargo check 2>&1 | tail -5</automated>
  </verify>
  <done>
    - `init_db` creates directory and opens SQLite with WAL mode
    - All three tables created via migrations (verified by unit test querying sqlite_master)
    - Settings CRUD works (get/set/defaults verified by unit tests)
