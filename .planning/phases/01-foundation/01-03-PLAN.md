...
<task type="auto">
  ...
  <action>
    ...
    **settings-panel.js**: Implement slide-out settings panel:
    ...
    - Three folder path inputs: Source, Organized, Originals
    - Each has a text input (editable) — folder picker dialog not available without tauri-plugin-dialog, so use text input for Phase 1. Note: Phase 1 uses manual path entry. Tauri folder dialog can be added later.
  ...
  <verify>
    <automated>ls src/js/panels/scan-table.js src/js/panels/settings-panel.js src/js/api.js src/js/main.js && echo "All frontend files exist"</automated>
  </verify>
  <done>
